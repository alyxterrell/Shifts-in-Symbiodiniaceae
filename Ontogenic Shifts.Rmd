...
#Histogram to measure for distribution
richness<-estimate_richness(ps2, measures = "Shannon")
hist(richness$Shannon, main = "Shannon Index", xlab = "") #It is


#Run anova test to check if variables impact diversity
#Treatment
anova.sh=aov(richness$Shannon~sample_data(ps2)$Treatment)
summary(anova.sh) # sig
#Time
anova.time=aov(richness$Shannon~sample_data(ps2)$TimePoint)
summary(anova.time) #significant
#Species
anova.sp=aov(richness$Shannon~sample_data(ps2)$Species)
summary(anova.sp) #significant

#Tukey to test for differences
TukeyHSD(anova.time) #T1-T0 and T2-T1
TukeyHSD(anova.sp)
TukeyHSD(anova.sh) #2100-2020 and 2100-2050


##Now again with relative abundance

richnessrel<-estimate_richness(psrel, measures = "Shannon")
hist(richness$Shannon, main = "Shannon Index", xlab = "") #It is

#Run anova test to check if variables impact diversity
#Treatment
anova.relsh=aov(richnessrel$Shannon~sample_data(psrel)$Treatment)
summary(anova.relsh) #not sig
#Time
anova.reltime=aov(richnessrel$Shannon~sample_data(psrel)$TimePoint)
summary(anova.reltime) #significant
#Species
anova.relsp=aov(richnessrel$Shannon~sample_data(psrel)$Species)
summary(anova.relsp) #significant

#Tukey to test for differences
TukeyHSD(anova.reltime) #T1-T0 and T2-T1, not T2-T0
...
```{r Significant Diversity Anova}
#Run anova test to check if variables impact diversity
#Treatment
anova.sh=aov(richness$Shannon~sample_data(ps2)$Treatment)
summary(anova.sh) #not sig
#Time
anova.time=aov(richness$Shannon~sample_data(ps2)$TimePoint)
summary(anova.time) #significant
#Species
